...
def AMPL(NMAX, DLAM, TL, TL1, PL, PL1, ALPHA, BETA):
    ...
    if (ALPHA < 0. or ALPHA > 360. or BETA < 0. or BETA > 180. or TL < 0. or TL > 180. or
        ...
        print(f'alpha = {ALPHA}, beta = {BETA}, tl = {TL}, tl1 = {TL1}, pl = {PL}, pl1 = {PL1}')
        quit()
...
def calculate(wavelength, Deq: float, epsilon, gamma, alpha_tilt, beta_tilt, theta: list):
    ...
    NDGS = 4  # Какой-то параметр точности интегрирования
...
#                print(M + 1, N2 + 1, N1 + 1, QSC)
   ...
   # quit()
    ...
    # test = (Z11 + Z22 + Z21 + Z12) / 2
    # print('PHASE', test, np.sqrt(test), np.abs(S11))
