...
@dataclass
class __EnsembleSettings:
    spheroid_model: SpheroidModel
    min_falling_angle: float
    increment_falling_angle: float
    min_temp: float
    increment_temp: float


def get_ensemble_generator(wavelength: float, type_ensemble: str):
    ensemble_models = []

    # Классы для сравнения с ФП из работы [Лю, Чандрасекар, 1999] в С-диапазоре частот
    if type_ensemble == 'drizzle_compare':
        ensemble_models = [__EnsembleSettings(SpheroidModel('drizzle', wavelength, 'compare'), 3., 4., -15., 20.)]
    if type_ensemble == 'rain_compare':
        ensemble_models = [__EnsembleSettings(SpheroidModel('rain', wavelength, 'compare'), 8., 6., -5., 10)]
    if type_ensemble == 'wet_snow_compare':
        ensemble_models = [__EnsembleSettings(SpheroidModel('wet_snow', wavelength, 'compare'), 8., 6., -15., 20)]
    if type_ensemble == 'dry_snow_compare':
        ensemble_models = [__EnsembleSettings(SpheroidModel('dry_snow', wavelength, 'compare'), 8., 6., -15., 15)]
    if type_ensemble == 'ice_crystals_compare':
        ensemble_models = [__EnsembleSettings(SpheroidModel('ice_crystals', wavelength, 'compare'), 4., 2., -15., 15)]

    len_ensemble_models = len(ensemble_models)

    def get_class_index_generator():
        if len_ensemble_models == 1:
            while True:
                yield 0
        else:
            while True:
                class_out = int(uniform(0, len_ensemble_models))
                if class_out < len_ensemble_models:
                    yield class_out
                else:
                    continue

    class_index_generator = get_class_index_generator()

    def generator_products():
        while True:
            class_out = next(class_index_generator)
            cur_model = ensemble_models[class_out]
            cur_temp = cur_model.min_temp + cur_model.increment_temp * random()
            cur_falling_ang = cur_model.min_falling_angle + cur_model.increment_falling_angle * random()
            Zh, Zdr, LDR, ro_hv, Kdp = cur_model.spheroid_model.get_products(cur_temp, cur_falling_ang)
            yield np.array([Zh, Zdr, LDR, Kdp], dtype=np.float64)
